...
for row in data:
    for col_name in row:
        ...
        if isinstance(row[col_name],list):
            if row[col_name] and isinstance(row[col_name][0],tuple) and isinstance(row[col_name][0][0],str):
...
PATH = 's3://aws-athena-query-results-411877231383-us-east-1/test_table_for_json_flatten/'
